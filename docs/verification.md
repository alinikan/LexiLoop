...
The follow-up repository audit corrected the Supabase instructions to use **Integrations → Database Webhooks → Install integration**, documented the `supabase_functions` provisioning error, added exact Vercel/Resend setup for deployments with and without a custom sending domain, and made signup-alert configuration an all-or-none optional group. Discover now ignores source, level and category filters during text search as documented, and uses memoized indexes for saved, dismissed, today and loaded words instead of repeatedly scanning growing collections. Node 24 is enforced, Next-managed `next-env.d.ts` is generated rather than tracked, formatting and source/build integrity are part of `npm run verify`, and CI now runs that complete command.
...
- `npm run verify`: integrity, formatting, lint, generated route types, TypeScript, the exact 5,000-card/unsafe-text audit, **125 tests in 14 files**, and the optimized Next.js 16 build.
- Embedded PostgreSQL executed all six real migrations, accent-insensitive cache normalization, the catalog metadata/batch seed functions, signup trigger/claim lease, migration idempotency, RLS and the existing learning transactions.
...
- A clean `npm ci` under Node 24 loaded the required native test binding. The audit found that filesystem synchronization in the current Documents location can restore stale numbered copies inside ignored `node_modules`, `.next` and test output after clean commands. Tracked source had no such copies. Generated output was removed/recreated for verification, and the new integrity check rejects numbered copies in source or build output. If copies return after `npm ci`, move the checkout to a non-synchronized development folder before reinstalling dependencies. Git status, formatting, conflict-marker checks and object connectivity remained healthy.
